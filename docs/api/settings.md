...
.. autoattribute:: Settings.dot_precision
   ...
   Precision mode for dot product operations. Default is typically ``"tf32"`` on CUDA and ``"ieee"`` on NPU. Controlled by ``TRITON_F32_DEFAULT`` environment variable.
...
|----------------------|---------|-------------|
| ``TRITON_F32_DEFAULT`` | ``dot_precision`` | Sets default floating-point precision for Triton dot products (``"tf32"``, ``"tf32x3"``, ``"ieee"``, ``"hf32"``). |
