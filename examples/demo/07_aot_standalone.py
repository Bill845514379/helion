"""AOT standalone compilation: self-invokes aot_runner with --standalone
to emit a .py with zero Helion dependencies.
"""
...
def benchmark_rms_norm() -> None:
    ...
    for b, h in [(256, 1024), (512, 2048), (1024, 4096)]:
...
if aot_mode != "disabled":
    ...
    output_dir = os.path.join(demo_dir, "07_aot_standalone_data")
    ...
    cmd = [sys.executable, "-m", "helion.experimental.aot_runner", "--standalone",
           "--output-dir", output_dir, "--single-config", "--", sys.executable, __file__]
    result = subprocess.run(cmd, env=env)

    standalone = os.path.join(demo_dir, "07_aot_standalone_rms_norm_standalone.py")
