...
scale = torch.tensor(127.0, device=DEVICE)

print("-- Approach 1: closure epilogue (works today) --")
print("Quantize fused inside the kernel, no extra memory traffic.\n")
bound = rms_norm.bind((x, w, lambda n, t: torch.clamp(torch.round(n * scale), -128, 127)))
print(bound.to_triton_code(CONFIG))

print("-- Approach 2: Inductor fusion (coming soon) --")
